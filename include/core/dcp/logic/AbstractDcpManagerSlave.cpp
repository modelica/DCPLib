#include "AbstractDcpManagerSlave.hpp"
...
void AbstractDcpManagerSlave::receive(DcpPdu &msg){
 ...
 */
void AbstractDcpManagerSlave::gotoErrorHandling() {
 ...
 */
void AbstractDcpManagerSlave::setError(DcpError errorCode) {
 ...
 * The master will be informed by NTF_state_changed.
 */
void AbstractDcpManagerSlave::gotoErrorResolved() {
...
AbstractDcpManagerSlave::AbstractDcpManagerSlave(const SlaveDescription_t _slaveDescription) : slaveDescription(_slaveDescription) {
...
void AbstractDcpManagerSlave::notifyStateChange() {
...
void AbstractDcpManagerSlave::setTimeRes(uint32_t numerator,
...
void AbstractDcpManagerSlave::setSteps(uint16_t dataId, uint32_t steps) {
...
void AbstractDcpManagerSlave::setOperationInformation(uint8_t dcpId, DcpOpMode opMode) {
    this->dcpId = dcpId;
#if defined(DEBUG)
...
void AbstractDcpManagerSlave::ack(uint16_t respSeqId) {
    DcpPduRspAck ack = {dcpId, respSeqId};
    driver.send(ack);
}

void AbstractDcpManagerSlave::nack(uint16_t respSeqId, DcpError errorCode) {
    const uint16_t expSeqId = segNumsIn[masterId] + 1;
    DcpPduRspNack nack = {DcpPduType::RSP_nack, dcpId, respSeqId, expSeqId, errorCode};
    driver.send(nack);
}

bool AbstractDcpManagerSlave::checkForError(DcpPdu &msg) {
    bool valid = true;
...
bool AbstractDcpManagerSlave::castAllowed(DcpDataType input, DcpDataType output) {
    switch (input) {
...
void AbstractDcpManagerSlave::clearOutputBuffer() {
...
void AbstractDcpManagerSlave::clearConfig() {
    driver.stop();
...
void AbstractDcpManagerSlave::updateStructualDependencies(uint64_t valueReference, size_t value) {
...
void AbstractDcpManagerSlave::checkForUpdatedStructure(uint64_t valueReference) {
...
void AbstractDcpManagerSlave::consume(const LogTemplate &logTemplate, uint8_t *payload, size_t size){
